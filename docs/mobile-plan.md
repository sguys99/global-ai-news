...
- [x] 상세 CTA를 floating-sticky-bar(64px frosted)로 격상 — 모바일은 하단 fixed frosted 바(`bg-background/80 backdrop-blur` + `border-t`, `paddingTop:0.75rem`+`paddingBottom:calc(0.75rem+safe-area)`, z-40)에 풀폭 pill CTA, 데스크톱은 인라인(`hidden md:inline-flex`). `/article/*`는 탭바 null(Phase 1)이라 충돌 없음. 본문 `pb-28 md:py-12`로 가림 방지
- [x] `aria-current="page"` 활성 탭, focus ring, reduced-motion — BottomTabBar 활성 탭에 `aria-current="page"` + `focus-visible:ring-1 ring-inset`, 상세 백바·CTA에 focus ring, `globals.css`에 `@media (prefers-reduced-motion:reduce)` 전역 transition/animation 무력화
- [x] DESIGN.md 최종 감사: 그림자 0(=`input.tsx` `shadow-xs` 제거 → 크롬 그림자 0), 단일 ink 강조(파란색 하드코딩 0 확인), weight 300/400/600/700(=앱 전역 `font-medium`(500) → `font-semibold`(600) 일괄 치환, 500 제거), frosted blur 값(크롬 `backdrop-blur`(8px)·시트 `/95` 유지)
- [ ] (선) 834px 커스텀 브레이크포인트 픽셀 정합 — **미적용(선택)**: `@theme` 커스텀 브레이크포인트 도입 + 전 구간 `md:` 재작성은 비용 대비 효익이 낮아 보류. 현재 `md:768` 유지
