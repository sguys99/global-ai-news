...
- [ ] `layout.tsx`에 `export const viewport` 추가 (`width:device-width`, `viewportFit:"cover"`, light/dark `themeColor`)
- [ ] body 하단 패딩 `pb-[calc(3.5rem+env(safe-area-inset-bottom))] md:pb-0` (탭바 가림 방지)
- [ ] `globals.css`에 `--tabbar-h` 토큰 + `@media (max-width:419px){ --text-display-md:28px }` 추가
- [ ] `Header.tsx` → `hidden md:flex`로 데스크톱 전용화
- [ ] `MobileTopBar.tsx` 신규 (`flex md:hidden`, 로고+검색+테마토글)
- [ ] `BottomTabBar.tsx` 신규: 2탭(피드/검색), frosted(`bg-background/80 backdrop-blur`)+hairline(`border-t`), active=`text-primary font-semibold`, safe-area 패딩, `/article/*`에서 `null`
- [ ] 검증: 모든 공개 페이지에서 모바일 상·하단 크롬 정상, 본문 비가림, 데스크톱 무변화

### Phase 2 — 터치 타겟 프리미티브
- [ ] `button.tsx` cva: `default:"h-11 ... md:h-9"`, `icon:"h-11 w-11 md:h-9 md:w-9"`
- [ ] `input.tsx`: `h-11 md:h-9`
- [ ] `FilterBar.tsx` chip: `min-h-11 md:min-h-0 py-2 md:py-1`
- [ ] 검증: 모바일에서 모든 인터랙션 컨트롤 ≥44px, 데스크톱 밀도 보존 (computed height 확인)

### Phase 3 — 피드 + 상세 모바일 다듬기
- [ ] 피드/상세/검색 컨테이너 `px-4 py-6 md:px-6 md:py-12`
- [ ] `ArticleCard.tsx` `p-4 md:p-6`
- [ ] 상세: `← 피드로` 링크를 모바일 sticky frosted 백바로 (`sticky top-0 ... md:static`)
- [ ] 상세: `원문 보기` CTA 모바일 풀폭 (`w-full justify-center md:w-fit`)
- [ ] 검증: 폰에서 피드·상세 가독성, 상세에서 하단 탭바 정상 숨김

### Phase 4 — 검색 필터 UX
- [ ] (선) 인라인 FilterBar 모바일 가로 스크롤(`flex-nowrap overflow-x-auto`) — className only 트레이서
- [ ] `FilterSheet.tsx` 신규: "필터" 트리거(≥44px, `md:hidden`) → 바텀시트(frosted+hairline+safe-area)에 **기존 FilterBar 재사용**
- [ ] 데스크톱 인라인 FilterBar는 `hidden md:block` 유지
- [ ] 검증: 모바일에서 칩 3줄 적층 없이 필터 사용 가능

### Phase 5 — 관리자 모바일
- [ ] `RunsTable.tsx`: `<table className="hidden md:table">` + `<div className="... md:hidden">` 카드 변형 (포매터·임계 하이라이트 공유, 단일 컴포넌트)
- [ ] `SourceManager.tsx`: 폼 필드 세로 스택, 추가/삭제 버튼 ≥44px
- [ ] admin 컨테이너 패딩 반응형화
- [ ] 하단 탭바 admin 처리: 공개 바엔 admin 탭 미노출(미인증 사용자 혼란/리다이렉트 방지), `/admin/*`는 자체 바 변형(`pathname.startsWith("/admin")`)
- [ ] 검증: 폰에서 관리자 콘솔 조작 가능
...
- `src/__tests__/BottomTabBar.test.tsx` — 탭바 active/숨김 테스트
